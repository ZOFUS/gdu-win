...
func TestStoredAnalyzer(t *testing.T) {
	...
	// On Windows, small files may report 0 usage; on Unix, minimum block size is 4096
...
func TestRemoveStoredFile(t *testing.T) {
	...
	assert.Equal(t, filepath.Join("test_dir", "nested", "subnested"), file.GetParent().GetPath())
}
