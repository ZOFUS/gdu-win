...
func processZipFile(zipPath string, info os.FileInfo) (zipDir *ZipDir, err error) {
	...
	for _, f := range reader.File {
		...
		// Normalize path to use forward slashes (standard for zip files)
		normalizedName := strings.ReplaceAll(f.Name, "\\", "/")

		// Parse file path and ensure all parent directories exist
		dirPath := ""
		lastSlash := strings.LastIndex(normalizedName, "/")
		if lastSlash > 0 {
			dirPath = normalizedName[:lastSlash]
		}
		...
		if lastSlash >= 0 {
			baseName = normalizedName[lastSlash+1:]
		}
...
func ensureZipDirExists(dirMap map[string]*ZipDir, path, zipPath string, rootDir *ZipDir) {
	...
	// Ensure parent directory exists - use path.Dir for forward slash handling
	parentPath := ""
