...
### Windows examples
    ...
    gdu.exe                               # analyze current dir
    gdu.exe -d                            # show all Windows drives (C:, D:, etc.)
    gdu.exe C:\Users                      # analyze specific directory
    gdu.exe -n -p .                       # non-interactive mode
    gdu.exe -o report.json -n .           # export to JSON
    gdu.exe -f report.json                # import from JSON
...
Gdu fully supports Windows. Features include:

* Display all Windows drives (C:, D:, etc.) with `-d` flag
* Full TUI support with keyboard navigation
* Shell spawning opens cmd.exe
* Windows paths with backslashes handled correctly
* JSON export/import preserves Windows paths
* ASCII progress bar used automatically (Unicode blocks don't render well in PowerShell)

Note: Some features like symlinks require elevated privileges on Windows.
