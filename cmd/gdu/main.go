...
func getDefaultIgnoreDirs() []string {
	if runtime.GOOS == "windows" {
		return []string{}
	}
	return []string{"/proc", "/dev", "/sys", "/run"}
}

func getDefaultStoragePath() string {
	if runtime.GOOS == "windows" {
		return filepath.Join(os.TempDir(), "gdu-badger")
	}
	return "/tmp/badger"
}
...
func init() {
	...
	flags.StringVarP(&af.LogFile, "log-file", "l", getDefaultLogFile(), "Path to a logfile")
	...
	flags.StringSliceVarP(&af.IgnoreDirs, "ignore-dirs", "i", getDefaultIgnoreDirs(),
	...
	flags.StringVar(&af.StoragePath, "storage-path", getDefaultStoragePath(), "Path to persistent key-value storage directory")
...
func runE(command *cobra.Command, args []string) error {
	...
	if af.LogFile == "-" {
		f = os.Stdout
	} else if af.LogFile == os.DevNull || af.LogFile == "NUL" || af.LogFile == "nul" {
		// On Windows, NUL cannot be opened with O_CREATE
